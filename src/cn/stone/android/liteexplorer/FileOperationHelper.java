...
public class FileOperationHelper {
	...
	synchronized private Handler getHandler() {
		if (mOperationHandler == null) {
			mOperationHandler = new Handler(getCommonThread().getLooper());
		}
		return mOperationHandler;
	}

	synchronized private HandlerThread getCommonThread() {
		if (mCommonThread == null || !mCommonThread.isAlive()) {
			mCommonThread = new HandlerThread("Common BG Thread");
			mCommonThread.start();
		}
		return mCommonThread;
	}
	...
	public boolean Rename(File f, String newName) {
		...
		String newPath = Util.makePath(Util.getPathFromFilepath(f.getPath()),
				newName);
	...
	protected void DeleteFile(File f) {
		...
		if (isdeleted && mOperationListener != null) {
			mOperationListener.onFileChanged(f.getPath());
		}
